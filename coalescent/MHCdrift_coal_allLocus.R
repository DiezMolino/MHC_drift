...
grid.size = 20
pop.size = round(4 * ((20000/4) ^ (1/(grid.size-1))) ^ (0:(grid.size-1)))
pop.size = sort(c(pop.size[pop.size %% 2 ==0], pop.size[pop.size %% 2 ==1]+1))
bot.size = round(4 * ((20000/4) ^ (1/(grid.size-1))) ^ (0:(grid.size-1)))
bot.size = sort(c(bot.size[bot.size %% 2 ==0], bot.size[bot.size %% 2 ==1]+1))
anc.size = 100000
nsims = 100
...
#r = log(bot.size[j]/pop.size[i])/10  #TO-DO pop. growth for 10 generations
#377 0 0 0 1 ', r, ' 0
...
212 11 0 1 1 0 0
242 12 0 1 1 0 0
381 0 0 0 ', BOTEND, ' 0 0
386 0 0 0 ', BOTINIT, ' 0 0
387 13 0 1 1 0 0
400 14 0 1 1 0 0
450 15 0 1 1 0 0
582 16 0 1 1 0 0
...
DNA 89 0.00000 0.0000001 0.33
DNA 92 0.00000 0.0000001 0.33')
write.table(parfile, file = paste0(files, '_', i, '_', j, '.par'), sep='\t',
    ...
    # look at the stats and calculate prob. of observing at least 67% alleles in the
    # wrangler population v ancestral population
    stats <- read.table(paste0('Stats_', files, '_', i, '_', j, '.txt'), quote="\"", comment.char="")
    p <- sum(stats[,2]*0.6315789 <= stats[,1])/nsims
    resmat[i,j] = p
...
par(fig=c(0,0.85,0,1), mar=c(5.5,5.5,4,1)+0.1, mgp=c(3.7,1,0))

image(matrix_1, xaxt='n', yaxt='n', col=pal.1(100), zlim=c(0, 0.05))
image(matrix_2, xaxt='n', yaxt='n', col=pal.2(100), zlim=c(0.05, 1), add=T)
axis(side=1, at=seq(0,1, length.out=grid.size), labels=pop.size/2, las=2, cex.axis=1)
axis(side=2, at=seq(0,1, length.out=grid.size), labels=bot.size/2, las=1, cex.axis=1)
title(main = paste0('Mammoth MHC | Exon 2, Intron 2a & Exon 4'), cex.main=1.5, ylab= "Ne at bottleneck",
      xlab= "Ne after bottleneck", cex.lab=1.5)
box()
...
mtext(text='Probability of observing 67% of ancestral alleles', side=4, line=1)
