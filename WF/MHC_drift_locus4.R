...
freq.init = c(0.20833, 0.20833, 0.29167, 0.33333, 0.20833, 0.3684211, 0.20833, 0.41667)
...
#       {
      ...
      resmat[i,j] = sum(apply(alleles.log, 1, sum) >= 6)/nsim
      cat(paste0('... pop.size ', i, ' out of ', length(pop.size), ' | bot.size ', j, ' out of ', length(bot.size), '\n'))
    }
}
write.table(resmat,
            file=paste0('resmat_', path, '_', grid.size,'.txt'),
            sep= '\t',
            col.names = F,
            row.names = F,
            quote = F,
...
pdf(paste0('resmat_', path, '_', grid.size,'.pdf'), width = 10, height = 8)
...
title(main =paste0('Exon 4 | ', n.polymor,' polymorphic positions'), cex.main=1.5, ylab= "Population size at bottleneck", xlab= "Population size after bottleneck", 
      cex.lab=0.8)
box()
...
mtext(text=paste0('Probability of observing ', n.all.wrangler, ' or more polymorphisms'), side=4, line=1)
